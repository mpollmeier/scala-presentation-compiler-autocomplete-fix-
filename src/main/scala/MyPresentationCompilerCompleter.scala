...
class MyPresentationCompilerCompleter(intp: IMain) extends Completion {
  import MyPresentationCompilerCompleter._
  ...
  override def complete(buf: String, cursor: Int): Candidates = {
    ...
    def tryAlternative = {
      // interpret everything up to last `.` and ask the compiler for completions on the result
      val beforeFinalDot = buf1.take(buf1.lastIndexOf("."))
      val valName = "__tmp__"
      intp.interpret(s"val $valName = $beforeFinalDot")
      val compilerResult = intp.presentationCompile(s"$valName.").right.get
      compilerResult.completionsAt(valName.length).results match {
        case Nil => None
        case results => Some(Candidates(cursor, results.map(_.sym.nameString).distinct.sorted))
      }
    }
    try {
      intp.presentationCompile(buf1) match {
        ...
        case Right(result) => try {
          buf match {
            ...
            case _ => candidates(result) match {
              case Completion.NoCandidates => tryAlternative.getOrElse(Completion.NoCandidates)
...
object MyPresentationCompilerCompleter {
